...
# new model
class EmnistFE(nn.Module):
    def __init__(self, input_dim, hidden_dims):
        super(EmnistFE, self).__init__()
        ...
        self.relu = nn.ReLU()
        self.fc1 = nn.Linear(input_dim, hidden_dims[0])   # input_dim = 16*4*4
        self.fc2 = nn.Linear(hidden_dims[0], hidden_dims[1])

    def forward(self, x):
        x = self.pool(self.relu(self.conv1(x)))    # -> [N,6,12,12]
        x = self.pool(self.relu(self.conv2(x)))    # -> [N,16,4,4]
...
class EmnistLettersCNN(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim=26):
        super(EmnistLettersCNN, self).__init__()
        self.base = EmnistFE(input_dim, hidden_dims)
        self.classifier = Classifier(hidden_dims[1], output_dim)
...
def emnist_letters_cnn(n_classes):
    return EmnistLettersCNN(input_dim=(16 * 4 * 4), hidden_dims=[120, 84], output_dim=n_classes)
