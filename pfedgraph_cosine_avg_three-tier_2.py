...
# load EMNIST-letters
train_local_dls, test_dl, net_dataidx_map = emnist_letters_get_dataloader(
...
for round in range(cfg["comm_round"]):
    ...
    for s_idx, client_ids in enumerate(server_groups):
        ...
        print(
            f">> Server {s_idx} | Round {round} | Server Test Acc (uniform over clients): {server_acc:.5f}"
        )
    ...
    if args.global_server_eval:
        ...
        print(
            f">> Global-Server | Round {round} | Global Test Acc (uniform over servers): {global_acc:.5f}"
        )
